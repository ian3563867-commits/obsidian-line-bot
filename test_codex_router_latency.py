...
project_id 必須完全等於有效 project 清單中的某一個資料夾名稱，不可只回代號，例如必須回「0182-SampleProjectB」而不是「0182」。
如果找不到對應專案，project_id 與 project_alias 請回空字串。
如果同一個 alias 可能對應多個 project，不要硬選；請降低 confidence，並在 reason 說明有多個候選。
topic 可以自由填寫短詞，例如：異常、MSG、SQL、用法、流程、SOP、人事、生活、規劃、資產。
...
def build_cmd(output_path: Path, schema_path: Path, model: str) -> list[str]:
...
def run_router(query: str, timeout: int, model: str) -> dict:
    project_ids = list_project_ids()
    prompt = build_router_prompt(query, project_ids)
    start = time.perf_counter()

    with tempfile.TemporaryDirectory(prefix="codex-router-") as tmp:
        tmp_dir = Path(tmp)
        output_path = tmp_dir / "router-output.json"
        schema_path = tmp_dir / "router-schema.json"
        schema_path.write_text(json.dumps(ROUTER_SCHEMA, ensure_ascii=False), encoding="utf-8")

        try:
            result = subprocess.run(
                build_cmd(output_path, schema_path, model),
                input=prompt,
                capture_output=True,
                text=True,
                encoding="utf-8",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Measure Codex router latency without vault tool use.")
    parser.add_argument("queries", nargs="*", help="Queries to route. Defaults to a small regression set.")
    parser.add_argument("--timeout", type=int, default=DEFAULT_TIMEOUT)
    parser.add_argument("--model", default=os.environ.get("CODEX_ROUTER_MODEL", ""))
    args = parser.parse_args()

    queries = args.queries or DEFAULT_QUERIES
    results = [run_router(query, args.timeout, args.model) for query in queries]
